...
⏱ **時間估算**：1 週（約 5 小時——不寫 code、只讀資源建進階概念地圖）
...
> 📋 **本章組成**：為什麼有這 stage → **第一個進階概念：Types → Config → Repo → Service 四層工作邊界**（diagram）→ 12 個進階概念 skeleton → 為什麼選這 12 個 → **核心 Harness Engineering 原則（多 source、4 大類別 + 關係圖）** → 進階 agentic 應用流程（5 step）→ 完整 reading path → 自我檢查
...
進階 agentic workflow 第一個值得學的設計概念是**工作邊界**——agent 操作的對象屬於 stack 哪一層？跨層越界會出什麼問題？
...
3. **套 5 個 OpenAI 原則（cross-cutting）**：Legibility / SoR / Progressive Disclosure / Taste Invariants / Throughput Merge Philosophy。這 5 原則跨越所有 12 概念、決定「做對與否」。
   → 本 stage §核心 Harness Engineering 原則
