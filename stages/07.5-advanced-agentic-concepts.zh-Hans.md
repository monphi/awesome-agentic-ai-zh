...
⏱ **时间估算**：1 周（约 5 小时——不写 code，只读资源建立进阶概念地图）
...
> 📋 **本章组成**：为什么有这 stage → **第一个进阶概念：Types → Config → Repo → Service 四层工作边界**（diagram）→ 12 个进阶概念 skeleton → 为什么选这 12 个 → **核心 Harness Engineering 原则（多 source、4 大类别 + 关系图）** → 进阶 agentic 应用流程（5 step）→ 完整 reading path → 自我检查
...
## 🧭 第一个进阶概念：四层工作边界（work boundary）

进阶 agentic workflow 第一个值得学的设计概念是**工作边界**——agent 操作的对象属于 stack 的哪一层？跨层越界会出什么问题？
...
把 agent 当作新进实习生来想：你交代一个明确的小任务、他却自作主张把邻近的东西也动了——这就叫「跨工作边界」。业界已有 3 个公开记录的真实案例可以对应：
...
- 最佳读物：[Hamel Husain — Evals + Skills](https://hamel.dev/blog/posts/evals-skills/) + [Cognition — Don't Build Multi-Agents](https://cognition.ai/blog/dont-build-multi-agents)（subagent context drift 案例）
...
1. **学第一个进阶概念 — 四层工作边界**：Types → Config → Repo → Service。想清楚 agent 能动到 stack 哪一层、跨层算违规。
   → 本 stage §四层工作边界
...
3. **套 5 个 OpenAI 原则（cross-cutting）**：Legibility / SoR / Progressive Disclosure / Taste Invariants / Throughput Merge Philosophy。这 5 条原则跨越全部 12 个概念、决定「做得对不对」。
   → 本 stage §核心 Harness Engineering 原则
